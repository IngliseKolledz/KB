...
var raneto = {
	...
	content_dir_for_lang: './content/et/',
	...
	getPage: function(filePath) {
		console.log('getPage', filePath);

		try {
			var file = fs.readFileSync(filePath),
				slug = filePath.replace(raneto.content_dir_for_lang, '').trim();
	...
	getPages: function(activePageSlug, language) {
		console.log('getPages', raneto.config.content_dir + '' + language +'/**/*');

		activePageSlug = activePageSlug || '';
		var page_sort_meta = raneto.config.page_sort_meta || '',
			category_sort = raneto.config.category_sort || false,
			files = glob.sync(raneto.config.content_dir + '/' + language +'/**/*'),
			filesProcessed = [];
		...
		files.forEach(function(filePath){
            var shortPath = filePath.replace(raneto.content_dir_for_lang, '').trim(),
			...
			if(stat.isDirectory()){
				...
				if(category_sort){
					try {
						var sortFile = fs.readFileSync(raneto.content_dir_for_lang + shortPath +'/sort');
	...
	doSearch: function(query, language) {
		var files = glob.sync(raneto.config.content_dir + language + '/**/*.md');
		...
		results.forEach(function(result){
            var page = raneto.getPage(raneto.content_dir_for_lang + result.ref);
